...
def answer_now_w_city(user_id):
    ...
    obs = owm.weather_at_coords(db_read(user_id)[5], db_read(user_id)[6])
    w = obs.get_weather()
    d = w.get_pressure()['press']
    drs = round(d / 1.333224)
...
def answer_today_f_city(user_id):
    owm = pyowm.OWM(api_key_pyowm, language='ru')
    fc = owm.three_hours_forecast_at_coords(db_read(user_id)[5], db_read(user_id)[6])
    f = fc.get_forecast()
    answer = '{0} 🏙️\n{1}, {2}\n'.format(
        db_read(user_id)[4],
...
def answer_tomorrow_f_city(user_id):
    owm = pyowm.OWM(api_key_pyowm, language='ru')
    fc = owm.three_hours_forecast_at_coords(db_read(user_id)[5], db_read(user_id)[6])
    f = fc.get_forecast()
...
def answer_in_2_days_f_city(user_id):
    owm = pyowm.OWM(api_key_pyowm, language='ru')
    fc = owm.three_hours_forecast_at_coords(db_read(user_id)[5], db_read(user_id)[5])
    f = fc.get_forecast()
...
def answer_in_3_days_f_city(user_id):
    owm = pyowm.OWM(api_key_pyowm, language='ru')
    fc = owm.three_hours_forecast_at_coords(db_read(user_id)[5], db_read(user_id)[6])
    f = fc.get_forecast()
...
def answer_in_4_days_f_city(user_id):
    owm = pyowm.OWM(api_key_pyowm, language='ru')
    fc = owm.three_hours_forecast_at_coords(db_read(user_id)[5], db_read(user_id)[6])
    f = fc.get_forecast()
...
def answer_in_5_days_f_city(user_id):
    owm = pyowm.OWM(api_key_pyowm, language='ru')
    fc = owm.three_hours_forecast_at_coords(db_read(user_id)[5], db_read(user_id)[6])
    f = fc.get_forecast()
